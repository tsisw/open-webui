<script lang="ts">
	...
	const mergeResponsesHandler = async () => {
		const responses = Object.keys(groupedMessageIds).map((modelIdx) => {
        	...
        	console.error("🚀 Calling mergeResponses with", messageId, responses, chatId);
		mergeResponses(messageId, responses, chatId);
                console.error("🚀 mergeResponsesHandler triggered with", { messageId, responses, chatId });
		...
		{#if !readOnly}
			{#if !Object.keys(groupedMessageIds).find((modelIdx) => {
				...
				<div class="flex justify-end">
					...
					console.error('🧪 Before isLastMessage =', isLastMessage);
					{#if isLastMessage}
						console.error('🧪 Inside isLastMessage =', isLastMessage);
							...
							<Tooltip content={$i18n.t('Merge Responses')} placement="bottom">
								<button
									...
									on:click={() => {
										...
										console.error('✅ mergeResponsesHandler clicked');
